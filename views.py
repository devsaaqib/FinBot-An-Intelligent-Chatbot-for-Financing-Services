...
def savemessage(request):
    ...
    user_detail = UserDetails.objects.filter(user=request.user).first()
    if not user_detail:
        return HttpResponse("UserDetails not found for the user", status=404)

    user_id = user_detail.pk

    message = request.GET.get("message", "")
    # Save or process the message as needed here...

    return JsonResponse({"status": "success", "user_id": user_id, "message": message}) 	
